...
class GameCog(commands.Cog):
    ...
    async def _trigger_encounter(
        ...
        """
        Spawn an encounter, post narration + initiative order, notify players,
        and run any leading monster turns. Called from both _maybe_trigger_combat
        and ActionsCog when the AI decides a fight should start.
        """
        from data.models import Monster
        from engine.combat_engine import start_encounter, get_turn_tracking
        from engine.action_validator import get_available_actions_text
        from ai.narrator import narrate_encounter_start
        from utils.embeds import combat_embed, narration_embed, info_embed
        ...
        monster_names = list({Monster.from_dict(m).name for m in combat.monsters.values()})
        ...
        if campaign.narration_channel_id:
            narr_ch = guild.get_channel(int(campaign.narration_channel_id))
            if narr_ch:
                await narr_ch.send(embed=narration_embed(
                    enc_narration, f"⚔️ {encounter_data.get('name', encounter_key)}"
                ))
                await narr_ch.send(embed=combat_embed("Initiative Order", order_str, "Combat begins!"))

        await db.append_log(guild_id, "combat",
            f"Encounter triggered: {encounter_data.get('name', encounter_key)} — {reason}")
                ...
                if ch:
                    ...
                    if char:
                        ...
                        await ch.send(embed=info_embed(
                            ...
                            + "\n\nUse the buttons below or slash commands to act."
                        ))

        # Run any leading monster turns (then notify the first player with CombatView)
        combat_cog = self.bot.get_cog("CombatCog")
        if combat_cog:
            await combat_cog._run_monster_turns(guild, campaign, combat)
    ...
    async def _maybe_trigger_combat(self, interaction: discord.Interaction, campaign, loc_data: dict):
        ...
        await self._trigger_encounter(
            interaction.guild, campaign, encounter_key, encounter_data, reason
        )
